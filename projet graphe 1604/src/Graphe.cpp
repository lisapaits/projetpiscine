...
void Graphe::affichage(BITMAP *buffer,int x_cst,int y_cst)
{
    ...
    for(int i=0;i<=m_ordre;i++)
    {
        for(int j=0;j<m_aretes.size();j++)
        {
            if(m_aretes[j].first==i)
            {
                P1=(m_aretes[j]).second->GetSommet1();
                P2=(m_aretes[j]).second->GetSommet2();
            }
        }
    ...
    textprintf_ex(buffer, font, 100+x_cst, 50+y_cst, makecol(255, 100, 200),-1, "(%f : %f)", somme_poids1(),somme_poids2());
        ...
        if((test_1==false)||(test_2==false))
        {
            for(int j=0;j<graph.GetArete().size();j++)
            {
                if(graph.GetArete()[j].first==k)
                {
                    ...
                    //m_aretes.insert({id_ar,new Arete{id_ar,som1,som2}});
                    std::pair<int,Arete*> tmp_ar;
                    tmp_ar.first=id_ar;
